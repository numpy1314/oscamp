...
fn main() {
    ...
    let mapping_flags = MappingFlags::READ | MappingFlags::WRITE | MappingFlags::EXECUTE;
    aspace.map_alloc(PHY_MEM_START.into(), PHY_MEM_SIZE, mapping_flags, true).unwrap();

    // Load corresponding images for VM.
    ax_println!("[h_4_1] VM created success, loading images...");
    ...
    // Setup VCpus.
    ax_println!("[h_4_1] bsp_entry: {:#x}; ept: {:#x}", KERNEL_BASE, aspace.page_table_root());
        ...
        match vcpu_run(&mut arch_vcpu) {
            Ok(exit_reason) => match exit_reason {
                ...
                AxVCpuExitReason::PageFault{addr, access_flags} => {
                    ax_println!("[h_4_1] PageFault: addr {:#x} access {:#x}", addr, access_flags);

                    let mapping_flags = MappingFlags::READ | MappingFlags::WRITE | MappingFlags::DEVICE;
                    let _ = aspace
                        .map_linear(
                            addr.align_down_4k(),
                            addr.align_down_4k().as_usize().into(),
                            4096,
                            mapping_flags,
                        )
                        .expect("map_linear failed");
                    ax_println!(
                        "[h_4_1] mapped addr {:#x} size {:#x} with flags {:#x}",
                        addr.align_down_4k(),
                        4096,
                        mapping_flags.bits()
                    );
...
fn vcpu_run(arch_vcpu: &mut AARCH64Vcpu) -> AxResult<AxVCpuExitReason> {
